...
\paragraph{Eine typographische Konvention.} Smalltalker nutzen hŠufig die Schreibweise ``\verb|>>|'', um die Klasse zu identifizieren, zu der eine Methode gehšrt. So wŸrde die Methode \ct{zellenProSeite}  in der Klasse \ct{SBESpiel}  beispielsweise durch \ct{SBESpiel>>zellenProSeite} referenziert werden. Um klarzustellen, dass das \emph{keine} \st Syntax ist, verwenden wir das Zeichen \ct{>>>} stattdessen, so dass diese Methode im Text als \ct{SBESpiel>>>zellenProSeite} erscheint.
\cmindex{Behavior}{>>}

Von jetzt an schreiben wir, wenn wir in diesem Buch eine Methode zeigen, den Namen der Methode in dieser Form.  NatŸrlich musst du, wenn du den Quelltext in den Browser schreibst, den Klassennamen oder \ct{>>>} nicht mitschreiben; statt dessen musst du nur sicherstellen, dass die entsprechende Klasse in der Klassenspalte ausgewŠhlt ist.

Lass uns nun die anderen beiden Methoden definieren, die in der  Methode \ct{SBESpiel>>>initialize}  benutzt werden. Beide kšnnen in das Protokoll \prot{initialization} eingefŸgt werden.
...
Diese Methode kšnnte kaum einfacher sein: sie antwortet mit der Konstanten 10. Ein Vorteil der Darstellung von Konstanten durch Methoden ist, dass die Methode geŠndert werden kann und den Wert der Konstanten berechnet, wenn das Programm sich so weiterentwickelt, dass die Konstante von anderen Eigenschaften abhŠngt.

\needlines{10}
\begin{method}[neueZelleBei:bei:]{Eine Hilfsmethode fŸr die Initialisierung}
SBESpiel>>>neueZelleBei: i bei: j
   "Erzeugt eine Zelle fŸr Position (i,j) und fŸgt sie am Bildschirm an der passenden Position ein. Gibt eine neue Zelle zurŸck."
   | c ursprung |
   c := SBEZelle new.
...
\dothis{FŸge die Methoden \ct{SBESpiel>>>zellenProSeite} and \ct{SBESpiel>>>neueZelleBei:bei:} hinzu.}
BestŠtige die Schreibweise der neuen Selektoren \ct{schalteNachbarnVonZelleBei:bei:} und \ct{mausAktion:}.

\Mthref{neueZelleBei:bei:} gibt eine neue SBEZelle zurŸck, speziell an Position \ct{(i, j)} in der \clsind{Matrix} der Zellen.
...
Eigentlich legt dies das Antwortverhalten fest, was ausgefŸhrt wird, wenn die Maustaste betŠtigt wird. Die korrespondierende Methode muss ebenfalls definiert werden.
...
\Mthref{schalteNachbarnVonZelleBei:bei:} schaltet den Zustand der vier Zellen im Norden, SŸden, Westen und Osten von Zelle (\ct{i}, \ct{j}) um.  Die einzige Komplikation liegt darin, dass das Spielfeld endlich ist und wir sicherstellen mŸssen, dass eine Nachbarzelle existiert, bevor ihren Zustand umschalten.
...
Um das Quinto-Spiel fertigzustellen, mŸssen wir noch zwei Methoden in der Klasse \ct{SBEZelle}  definieren, die sich um die Mausereignisse kŸmmern.
...
\Mthref{mausAktion:}  macht weiter nichts als der Variablen \ct{mausAktion} der Zelle das Argument zuzuweisen und dann den neuen Wert zurŸckzugeben.
Eine Methode die den Wert einer Instanzvariable \emph{Šndert}, wird \emph{setter Methode} genannt; eine Methode die den aktuellen Wert einer Instanzvariable \emph{zurŸckgibt}, hei§t \emph{getter-Methode}.

\seeindex{setter method}{accessor}
\seeindex{getter method}{accessor}

Wenn du getter und setter aus anderen Programmiersprachen kennst, erwartest du vielleicht, dass sie \ct{setmausAktion} und   \ct{getmausAktion} hei§en. Die  \st  Konventionen sind anders. Ein getter hat immer den selben Namen wie die Variable, die er liest und ein setter ist Šhnlich benannt, er hat nur ein angehŠngtes ``\ct{:}'', also  \ct{mausAktion} und \ct{mausAktion:}.
...
In Smalltalk sind \emph{alle} Instanzvariablen in dem Objekt gekapselt, dem sie angehšren, so dass der einzige Weg fŸr andere Objekte, diese Variablen in Smalltalk zu lesen oder zu schreiben in derartigen accessor-Methoden besteht \footnote{TatsŠchlich kann auf die Instanzvariablen auch in Unterklassen zugegriffen werden.}.


\dothis{Gehe zur Klasse \ct{SBEZelle}, definiere \ct{SBEZelle>>>mausAktion:} und lege sie in das \prot{accessing} Protokoll.}

Letztendlich mŸssen wir noch eine Methode \ct{mouseUp:} definieren; diese wird automatisch von der GUI-Umgebung aufgerufen, wenn die Maustaste losgelassen wird, wŠhrend die Maus sich Ÿber dieser Zelle auf dem Bildschirm befindet.

\begin{method}[sbezellzmouseup]{Eine Ereignisbehandlung}
SBECell>>>mouseUp: anEvent
   mausAktion value
\end{method}
\cmindex{SBEZelle}{mouseUp:}

\dothis{FŸge die Methode \ct{SBEZelle>>>mouseUp:} hinzu und kategorisiere sie mit \menu{categorize all uncategorized} methods.}
\index{method!categorize}
...
\noindent
Das Senden der Nachricht \ct{value} veranlasst die AusfŸhrung dieses Quelltext-Fragmentes und wird so konsequenterweise den Zustand der Zellen umschalten.

%=================================================================
\section{Lass uns unseren Quelltext ausfŸhren}
...
der Debugger zeigt den  \ind{AusfŸhrungskontext} in dieser Methode, wo der Fehler auftrat
(\figref{debugToggle}).
...
\caption{Der Debugger, mit der Methode \ct{schalteNachbarnVonZelleBei:bei:} ausgewŠhlt.
